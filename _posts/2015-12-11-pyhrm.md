...
but after a quick skim of the Wikipedia article on base64 I got things working:
...
[Note that 65536 = 2^16, the largest unsigned 16-bit integer value.]
...
my script to parse and render the drawings from a Human Resource Machine progam
using Python's built-in `turtle` drawing library. It's not particularly fast,
but it does make a cool animated effect.
